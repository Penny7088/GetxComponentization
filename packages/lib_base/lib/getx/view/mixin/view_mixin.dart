...
mixin ViewMixin {
  ///   创建流程
  ///   基类 调用 [createScaffoldWidget] -> 是否使用脚手架 是 -> [Scaffold]->[body]->[createScaffoldBody]->[createSafeArea]->->[createSafeBody]->[createContentBody]
  ///                                                  否 -> [createScaffoldBody]->[createSafeArea]->->[createSafeBody]->[createContentBody]
  ///

  Widget createScaffoldWidget({
    ...
    body = configIsNeedScaffold()
        ? Scaffold(
            ...
            backgroundColor: configScaffoldBackgroundColor(context),
            resizeToAvoidBottomInset: configResizeToAvoidBottomInset(),
            body: body,
            bottomNavigationBar: configIsNeedBottomNavigation()
                ? createScaffoldBottomNavigationBar(context: context)
                : null,
          )
        : body;
    return body;
  ...
  Widget createSafeArea({
    ...
    Widget body = createSafeBody(context: context, constraints: constraints);
  ...
  Widget createSafeBody({
    ...
    Widget body = createContentBody(constraints: constraints, context: context);
